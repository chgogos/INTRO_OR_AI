...
def solve_word_equation(word1, word2, result_word):
    ...
    letter_vars = {letter: model.NewIntVar(0, 9, letter) for letter in unique_letters}
    ...
    class SolutionPrinter(cp_model.CpSolverSolutionCallback):
        ...
        def OnSolutionCallback(self):
            ...
            w1_val = sum(assignment[l] * (10 ** i) for i, l in enumerate(reversed(self.word1)))
            w2_val = sum(assignment[l] * (10 ** i) for i, l in enumerate(reversed(self.word2)))
            res_val = sum(assignment[l] * (10 ** i) for i, l in enumerate(reversed(self.result_word)))
            print(f"{self.word1} ({w1_val}) - {self.word2} ({w2_val}) = {self.result_word} ({res_val})")
